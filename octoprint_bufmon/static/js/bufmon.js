...
$(function() {
    function BufmonViewModel(parameters) {
        ...
        self.onEventplugin_bufmon_buffer_data = function(payload) {
            self.plannerHist = [];
            for(var v in payload.planner_hist) {
                self.plannerHist.push({ value: v, frequency: payload.planner_hist[v] });
            }

            self.inputHist = [];
            for(var v in payload.input_hist) {
                self.inputHist.push({ value: v, frequency: payload.input_hist[v] });
            }

            self.plannerChart.update(self.plannerHist);
            self.inputChart.update(self.inputHist);
        }

        self.onStartupComplete = function() {
            // setup the bar chart
            self.plannerChart = BarChart(self.plannerHist, {
                x: d => d.value,
                y: d => d.frequency,
                yLabel: "Planner Buffer",
                width: 500,
                height: 250,
                color: "steelBlue",
                duration: 250
            });
            $("#bufmon-planner-hist").append(self.plannerChart);

            self.inputChart = BarChart(self.inputHist, {
                x: d => d.value,
                y: d => d.frequency,
                yLabel: "Input Buffer",
                width: 500,
            ...
            $("#bufmon-input-hist").append(self.inputChart);
        }
    }
